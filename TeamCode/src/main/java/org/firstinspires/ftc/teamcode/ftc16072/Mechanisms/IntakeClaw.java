...
public class IntakeClaw extends QQMechanism {
    ...
    public void setWristAngle(double angle){
        double servoAngle = WRIST_START_POSITION + (WRIST_TRANSFER_POSITION - WRIST_START_POSITION) * (angle/300);
        if(telemetry != null){
            telemetry.addData("angle", angle);
            telemetry.addData("ServoAngle", servoAngle);
        }
        leftWristServo.setPosition(1-servoAngle);
        rightWristServo.setPosition(servoAngle);
    }
